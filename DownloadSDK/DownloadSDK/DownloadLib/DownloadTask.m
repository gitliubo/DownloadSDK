...
- (instancetype)initWithUrl:(NSString *)url fileId:(NSString *)fileId delegate:(id<DownloadTaskDelegate>)delegate {
...
+ (instancetype)taskWithUrl:(NSString *)url fileId:(NSString *)fileId delegate:(id<DownloadTaskDelegate>)delegate {
    return [[DownloadTask alloc] initWithUrl:url fileId:fileId delegate:delegate];
}
...
- (void)resume {
    [DownloadSession.downloadSession resumeDownloadWithTask:self];
}

- (void)pause {
    [DownloadSession.downloadSession pauseDownloadWithTask:self];
}

- (void)remove {
    [DownloadSession.downloadSession stopDownloadWithTask:self];
}

#pragma mark - getter

-(NSString *)taskId {
    return [DownloadTask taskIdForUrl:self.downloadURL fileId:self.fileId];
}
...
- (NSString *)saveName {
    if (_saveName.length==0) {
        NSString *name = [DownloadTask taskIdForUrl:self.downloadURL fileId:self.fileId];
        NSString *pathExtension =  [DownloadTask getPathExtensionWithUrl:self.downloadURL];
...
- (void)setSaveName:(NSString *)saveName {
    _saveName = saveName;
    [DownloadSession.downloadSession saveDownloadStatus];
}

+ (NSString *)taskIdForUrl:(NSString *)url fileId:(NSString *)fileId {
    NSString *name = [DownloadTask md5ForString:fileId.length>0 ? [NSString stringWithFormat:@"%@-%@",url, fileId] : url];
    return name;
}
...
+ (NSString *)saveDir {
    ...
    saveDir = [saveDir stringByAppendingPathComponent:@"MozLite/MultimediaDownloadFile"];
